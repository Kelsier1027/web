...
import { filter, map, mergeMap, of, from, switchMap, tap, catchError } from 'rxjs';
import { promoTagLabel } from 'src/app/constants/product.constants';
import { StorageService } from 'src/app/core/services/storage.service';
import { DialogAction, PromoMethod, ResponseCode } from 'src/app/enums';
import { CreateOrderEnum } from 'src/app/enums/order.enum';
import { StorageEnum } from 'src/app/enums/storage.enum';
import { City, CityArea, AddToCartReplaceItem } from 'src/app/models';
import { ProductService, MemberService } from 'src/app/services';
...
import { PeriodicElement } from '../checkout-process/checkout-process.component';
import { STRING_UTIL } from 'src/app/shared/utils/stringUtilities';

@Component({
  selector: 'app-preorder-process',
  templateUrl: './preorder-process.component.html',
  styleUrls: ['./preorder-process.component.scss'],
  providers: [CheckoutUtilService],
})
export class PreorderProcessComponent implements OnInit {
  qty: number = 0;
  ...
  constructor(
    ...
    this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        const checkoutList: any = this.storageService.get(StorageEnum.Preorder);
        const {
          addToCart,
          subInventory,
          ...
          awards,
          commonlyUsedAddr, // 2024/11/26 Add by Tako for 2024024202
        } = checkoutList;
        const products: any[] = [];
        ...
        this.inputDividendDiscount = this.form.isChecked
          ? usableBonusPoints
          : 0;
        this.awardGiftListIds = awards;
        this.specifiedDeliveryFeeExemptedThreshold =
          specifiedDeliveryFeeExemptedThreshold;
        this.specifiedDeliveryFee = specifiedDeliveryFee;
        this.commonlyUsedAddrLength = checkoutList.commonlyUsedAddr.length;
        this.thisHasPurchaserRemarks = checkoutList.hasPurchaserRemarks;
        this.thisLatestPurchaserRemark = checkoutList.latestPurchaserRemark;
        this.purchaserJobTitle = checkoutList.purchaserJobTitle;
        this.purchaserName = checkoutList.purchaserName;
        this.awards = checkoutList.awards;
        this.form.email = checkoutList.purchaserEmail;
        this.defaultBarcode = checkoutList.defaultBarcode || '';
        this.isQualifiedForCloudInvoice = checkoutList.isQualifiedForCloudInvoice;
        ...
        this.shippingMethods = this.createShippingMethods(checkoutList);
  ...
  getPromotionTag(promotionMethod: number, promoCategory: number, tagName?: string) {
    if (tagName === "預購") {
      return {
        text: '預購',
        label: '預購',
    ...
    if (!promoCategory) {
      return promoTagLabel[1][promotionMethod];
    }
  ...
  submit() {
    ...
    const param = {
      addToCart: {
        itemId: this.addToCart.itemId,
        promoId: this.addToCart.promoId,
        orderQuantity: this.addToCart.orderQuantity,
        subInventoryCode: this.addToCart.subInventoryCode,
      },
    ...
    this.productService
      ...
      .subscribe((res: any) => {
        if (res.responseCode === '0000') {
          this.storageService.set(StorageEnum.PreOrderCheckoutResult, res.result);
          this.router.navigate(['/OrderComplete'], { queryParams: { type: 'preOrder' } })
            .then(() => {
              window.location.reload(); //下個頁面會吃到此頁的css，先重整一次
            });
